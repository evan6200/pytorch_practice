...
if (len(sys.argv) <=1 ):
  pkl='0605_demo_uni_vector_X_EPOCH.pkl'
else:
  pkl=sys.argv[1]
...
def draw_CLASS_statistic(class_rate):
  ...
  filename=pic_location+'/DEMO_CLASS_ACC_7P_1M9M_unitV_' + date+'.png'
...
def draw_METER_statistic(Meter_rate):
  ...
  plt.show()
  date=time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())
  filename=pic_location+'/DEMO_METER_ACC_9P_1M9M_unitV_' + date+'.png'
  plt.savefig(filename)
