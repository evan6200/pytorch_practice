...
#training_keypoint=k_MIX_7PERSON
#training_keypoint=k_MIX_7PERSON_EVAN
training_keypoint=k_MIX_7PERSON
#training_keypoint=k_EVAN_1_9_WITH_CLOTHES
    ...
    def __init__(self, initial_num_channels, num_classes, num_channels):
        ...
        self.convnet = nn.Sequential(
            nn.Conv1d(in_channels=initial_num_channels,
                      out_channels=100, kernel_size=5,stride=2),                      
            nn.ReLU(inplace=True),
            nn.Conv1d(in_channels=100, out_channels=160,
                      kernel_size=5,stride=2),
            nn.ReLU(inplace=True),
            #torch.nn.Dropout(0.2),
            nn.Conv1d(in_channels=160, out_channels=160,
                      kernel_size=3,stride=2),
            nn.ReLU(inplace=True),
            nn.MaxPool1d(3, stride=2),
            torch.nn.Dropout(0.5),
            nn.ReLU(inplace=True),         
            nn.Conv1d(in_channels=160,
...
#    print('Train--loss=',loss,'ave_loss=',ave_loss,'correct_cnt=' ,correct_cnt,'total_cnt=', total_cnt)
    print('Train LOSS->',loss_cpu.item())
    ...
    #print('Evan print TEST ACC',ave_test_acc,'VAL LOSS',ave_loss,'total_cnt',total_cnt)
...
#    out_all_ERR=dataE
    ...
    print ('val ACC=',acc,'val loss',ave_loss,'train_acc',train_acc)

    #if acc>0.70 and ave_loss < 0.4 and train_acc > 0.75:
    if acc > 0.65 and  train_acc > 0.89:
        break
torch.save(net,"0427_uni_vector_NET_TUEINE.pkl")
if (PLT_UI==True):
