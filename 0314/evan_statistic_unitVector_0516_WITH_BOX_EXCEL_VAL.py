...
if (len(sys.argv) <=1 ):
  ...
  pkl='0516_pkl/0501_uni_vector_2020-05-24_11:16:11.pkl'
else:
  pkl=sys.argv[1]
...
#          X=X.unsqueeze(1) # the data formate should be [batch_size,1,30]
          ...
          for i, value in enumerate(pred_label):
            ...
            if (value.item()==1 or value.item()==4):
              ans_pred=shoulder_norm[i]/arcsine[0] # 22.5
            if (value.item()==2 or value.item()==5):
              ans_pred=shoulder_norm[i]/arcsine[1] # 45
            if (value.item()==3 or value.item()==6):
              ans_pred=shoulder_norm[i]/arcsine[2] # 80 67.5
            #value.item()=pred
                          
            ans_COL.append(ans)  #finish recover shoulder distance to class0
            ans_pred_COL.append(ans_pred)
            if(y[i]==value.item()):
              correct=correct+1
              result_col.append(1)
              #result_fix_col.append(1)  #for fixed result.
              pred_col.append(9)
            else:
              result_col.append(0)
              pred_col.append(value.item())
            #fixed ground true 0=1||4  1=2|0
            if((y[i]==0 and (value.item()==1 or value.item()==4 or value.item()==0))):
              result_fix_col.append(1)
            elif((y[i]==1 and (value.item()==0 or value.item()==2 or value.item()==1))):
              result_fix_col.append(1)
            elif((y[i]==2 and (value.item()==1 or value.item()==3 or value.item()==2))):
              result_fix_col.append(1)
            elif((y[i]==3 and (value.item()==2 or or value.item()==3))):
              result_fix_col.append(1)
            elif((y[i]==4 and (value.item()==0 or value.item()==5 or value.item()==4))):
              result_fix_col.append(1)
            if((y[i]==5 and (value.item()==4 or value.item()==6 or value.item()==5))):
              result_fix_col.append(1)
            if((y[i]==6 and (value.item()==5 or value.item()==6))):
              result_fix_col.append(1)
            else:
              result_fix_col.append(0)
                  
          result_row.append(result_col)
          result_shoulderROW.append(result_shoulderCOL)
          ans_ROW.append(ans_COL)
          pred_row.append(pred_col)
          result_fix_row.append(result_fix_col)
          ans_pred_ROW.append(ans_pred_COL)
