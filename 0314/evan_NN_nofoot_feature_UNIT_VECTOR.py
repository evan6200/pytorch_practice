...
def in_feature(keypoints):
  ...
  min_dist=[]
  count=0
  #0501 add solution for multi person START
  ...
  if (len(keypoints)>1):
    for a in keypoints:
      neckX=a[1][0] #neck X
      neckY=a[1][1] #neck Y
      neck= np.array((float(neckX),float(neckY))) 
      center = np.array((float(960),float(540)))
      min_dist.append(get_dist(neck,center)) 
    center_person_index=min_dist.index(min(min_dist))  
    #replace keypoints to center one
    center_person.append(keypoints[center_person_index])
  else:
    center_person=keypoints
  for a in center_person:  #0501 add solution for multi person END
