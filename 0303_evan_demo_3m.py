...
def draw_prediction(_img,start,end,pred):
    img2 = _img
    fontsize=2
    x1,y1,x2,y2=start[0],start[1],end[0],end[1]
    moved=(np.linalg.norm(end-start)/4)
    x2=x1+moved
    y1=y1+moved
    y2=y2+moved
    if pred_result==0:
        ...
        cv2.putText(img2, str(pred_result), (int(x1), int(y1+moved)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255,0), fontsize, 1)
...
net =torch.load('/home/evan/mp4_to_png/1219_3m1p6m32p.pkl')
net.to(device)
    ...
    while(cap.isOpened()):
      ...
      if ret == True:
        ...
        if(len(x) == 25):
          ...
          x=[]
#        cv2.imshow('Frame',datum.cvOutputData)
        noseX,noseY=datum.poseKeypoints[0][0],datum.poseKeypoints[0][1]
        print('noseX',noseX,'noseY',noseY)

        start= np.array((float(noseX),float(noseY))) 
        end=np.array((float(noseX)+100,float(noseY)))   
        draw_prediction(datum.cvOutputData,start,end,np.argmax(a))
        cv2.imshow('Frame',datum.cvOutputData)
